...
class ProfilePage(ctk.CTkFrame):
    ...
    def on_show(self) -> None:
        ...
        if not user:
            messagebox.showerror(common_txt["error"], "Please log in first.")
        ...
        # Pull statistics from the DB and render a compact summary line
    ...
    def save_changes(self) -> None:
        ...
        if not new_username or not new_email:
            messagebox.showerror(common_txt["error"], "Fields cannot be empty.")
            return

        if self.db.update_user(user["id"], new_username, new_email):
            # Keep controller state in sync with the database
            self.controller.current_user = {"id": user["id"], "username": new_username, "email": new_email}
            messagebox.showinfo(common_txt["success"], "Profile updated!")
            self.on_show()
        else:
            messagebox.showerror(common_txt["error"], "Username/email is already in use.")
    ...
    def update_texts(self) -> None:
        ...
        # Refresh current values when the language changes
