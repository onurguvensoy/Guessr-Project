...
class LobbyScreen(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.scroll_frame = ctk.CTkScrollableFrame(
            self, height=250, label_text="Select Playlist", fg_color="transparent"
        )
        ...
        self.round_combo.set("3")  # safe default
        ...
        self.timer_combo.set("No Limit")  # safe default
        self.timer_combo.grid(row=0, column=3, padx=5)

        # No-pan / no-move switch
        self.no_move_var = ctk.BooleanVar(value=False)
        self.no_move_switch = ctk.CTkSwitch(
            self.settings_card,
            text="Disable Navigation (No Move)",
            variable=self.no_move_var,
            font=("Impact", 14),
        ...
        self.create_btn = ctk.CTkButton(
            self,
            text="CREATE PLAYLIST / MAP",
            command=lambda: controller.show_frame("CreatePlaylistPage"),
            fg_color="#1F6AA5",
        )
    ...
    def on_show(self) -> None:
        ...
        for playlist_name in playlists:
            btn = ctk.CTkButton(
                self.scroll_frame,
                text=playlist_name,
                fg_color=("#D1E8FF", "#2B2B2B"),
                text_color=("#1F6AA5", "white"),
                command=lambda n=playlist_name: self.select_playlist(n),
            )
            btn.pack(pady=2, fill="x", padx=10)

    def select_playlist(self, name: str) -> None:
        """Store the selection and update the header to reflect the active map."""
    ...
    def start_game(self) -> None:
        self.controller.game_config = {
            ...
            "no_move": self.no_move_var.get(),  # forwarded to JS through GameApi.get_state()
        }
